...
PopGenModelsController::PopGenModelsController( RobotWorldModel *wm )
{
    _wm = wm;
    ...
    _currentSigma = PopGenModelsSharedData::gSigmaRef;
    ...
    _isListening = true;
    _notListeningDelay = PopGenModelsSharedData::gNotListeningStateDelay;
    _listeningDelay = PopGenModelsSharedData::gListeningStateDelay;
...
PopGenModelsController::~PopGenModelsController()
{
    _parameters.clear();
    delete nn;
    nn = NULL;
...
void PopGenModelsController::reset()
{
    _parameters.clear();
    _parameters = _genome;
}
...
void PopGenModelsController::step()
{
    _iteration++;
...
void PopGenModelsController::stepBehaviour()
{
    ...
    if ( PopGenModelsSharedData::gEnergyRequestOutput )
    {
...
void PopGenModelsController::createNN()
{
    if ( nn != NULL ) // useless: delete will anyway check if nn is NULL or not.
        delete nn;
    
    switch ( PopGenModelsSharedData::gControllerType )
    {
        case 0:
        {
            // MLP
        ...
        default: // default: no controller
            std::cerr << "[ERROR] gController type unknown (value: " << PopGenModelsSharedData::gControllerType << ").\n";
            exit(-1);
    };
}


unsigned int PopGenModelsController::computeRequiredNumberOfWeights()
{
...
void PopGenModelsController::stepEvolution()
{
    ...
    if( gWorld->getIterations() > 0 && gWorld->getIterations() % PopGenModelsSharedData::gEvaluationTime == 0 )
                    //dynamic_cast<PopGenModelsWorldObserver*>(gWorld->getWorldObserver())->getGenerationItCount() == 0 )  //todelete
    {
        loadNewGenome();
        _nbGenomeTransmission = 0;
    }
    ...
    if ( PopGenModelsSharedData::gLogGenome && gWorld->getIterations() % ( PopGenModelsSharedData::gEvaluationTime * PopGenModelsSharedData::gSnapshotsFrequency ) == 1 )
    {
        // Logging: full genome
        std::string sLog = std::string("");
...
void PopGenModelsController::selectFirstGenome()
{
    if(_genomesList.size() != 0)
    {
...
bool PopGenModelsController::storeGenome(std::vector<double> genome, int senderId, int senderBirthdate, float sigma)
{
...
void PopGenModelsController::mutate( float sigma) // mutate within bounds.
{
    _genome.clear();
...
void PopGenModelsController::resetRobot()
{
    _nbInputs = 0;
    ...
    _nbHiddenLayers = PopGenModelsSharedData::gNbHiddenLayers;
    
    _nbNeuronsPerHiddenLayer = new std::vector<unsigned int>(_nbHiddenLayers);
    for(unsigned int i = 0; i < _nbHiddenLayers; i++)
        (*_nbNeuronsPerHiddenLayer)[i] = PopGenModelsSharedData::gNbNeuronsPerHiddenLayer;
    ...
    {
        _genome.push_back((double)(rand()%PopGenModelsSharedData::gNeuronWeightRange)/(PopGenModelsSharedData::gNeuronWeightRange/2)-1.0); // weights: random init between -1 and +1
    }
    _currentGenome = _genome;
    setNewGenomeStatus(true);
    _genomesList.clear();
...
void PopGenModelsController::broadcastGenome()
{
    // remarque \todo: limiting genome transmission is sensitive to sensor order. (but: assume ok)
    
    for( int i = 0 ; i < _wm->_cameraSensorsNb && ( PopGenModelsSharedData::gLimitGenomeTransmission == false || ( PopGenModelsSharedData::gLimitGenomeTransmission == true && _nbGenomeTransmission < PopGenModelsSharedData::gMaxNbGenomeTransmission ) ); i++)
    {
        ...
        if ( targetIndex >= gRobotIndexStartOffset )   // sensor ray bumped into a robot : communication is possible
        {
            ...
            PopGenModelsController* targetRobotController = dynamic_cast<PopGenModelsController*>(gWorld->getRobot(targetIndex)->getController());
            ...
            if ( targetRobotController->isListening() )
            {
                ...
                if ( dice <= PopGenModelsSharedData::gProbaMutation )
                {
                    dice = float(rand() %100) / 100.0;
                    if ( dice < 0.5 )
                    {
                        sigmaSendValue = _currentSigma * ( 1 + PopGenModelsSharedData::gUpdateSigmaStep ); // increase sigma
                        
                        if (sigmaSendValue > PopGenModelsSharedData::gSigmaMax)
                        {
                            sigmaSendValue = PopGenModelsSharedData::gSigmaMax;
                        }
                    }
                    else
                    {
                        sigmaSendValue = _currentSigma * ( 1 - PopGenModelsSharedData::gUpdateSigmaStep ); // decrease sigma
                        
                        if ( sigmaSendValue < PopGenModelsSharedData::gSigmaMin )
                        {
                            sigmaSendValue = PopGenModelsSharedData::gSigmaMin;
                        }
                    }
...
void PopGenModelsController::loadNewGenome()
{
    if ( _wm->isAlive() || gEnergyRefill )  // ( gEnergyRefill == true ) enables revive
    {
        ...
        if (_genomesList.size() > 0)
        {
            ...
            switch ( PopGenModelsSharedData::gSelectionMethod )
            {
                case 0:
                    selectRandomGenome();
                    break;
                case 1:
                    ...
                    break;
                default:
                    std::cerr << "[ERROR] unknown selection method (gSelectionMethod = " << PopGenModelsSharedData::gSelectionMethod << ")\n";
                    exit(-1);
            }
        ...
        else
        {
            ...
            if ( _wm->isAlive() == true )
            {
                ...
                if ( PopGenModelsSharedData::gNotListeningStateDelay != 0 ) // ie. -1 (infinite,dead) or >0 (temporary,mute)
                {
                    _isListening = false;
...
void PopGenModelsController::logCurrentState()
{
    // Logging
